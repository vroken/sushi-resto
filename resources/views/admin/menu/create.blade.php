                ...
                <div class="overflow-x-auto sm:-mx-6 lg:-mx-8">   
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg inline my-5">
                        <form method="POST" action="{{ route('admin.menus.store') }}" enctype="multipart/form-data">
                                ...
                                <div class="mt-1">
                                    <div class="flex items-center">
                                        <input type="file" id="image" name="image"
                                            class="block w-full appearance-none bg-grey-900 border border-gray-400 rounded-md py-3 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 text-white @error('image') border-red-400 @enderror" onchange="previewImage()"/>
                                    </div>
                                    <img class="img-preview object-contain mt-3" alt="" >
                                </div>
                                @error('image')
                                    <div class="text-sm text-red-400">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="sm:col-span-6 pt-5 mt-6">
                                <label for="description" class="block text-sm font-medium text-white">Deskripsi</label>
                                <div class="mt-1">
                                    <textarea id="description" rows="3" name="description"
                                        class="block w-full appearance-none border-white-300 dark:border-white-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 @error('name') border-red-400 @enderror"></textarea>
                                </div>
                                @error('description')
                                    <div class="text-sm text-red-400">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="sm:col-span-6 mt-6">
                                ...
                                <div class="mt-1">
                                    <input type="number" id="price" name="price"
                                        class="block w-full appearance-none border-white-300 dark:border-white-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 price-input @error('price') border-red-400 @enderror"
                                        placeholder="Rp." />
                                </div>                                
                                @error('price')
                                    <div class="text-sm text-red-400">{{ $message }}</div>
                            ...
                            <div class="mt-6">
                                <button type="submit"
                                    class="px-8 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Tambah Menu</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
    ...
    //         var sanitizedValue = $(this).val().replace(/[^0-9]/g, '').replace(/^0+/, '');
